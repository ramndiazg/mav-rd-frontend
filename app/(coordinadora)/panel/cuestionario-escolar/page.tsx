...
// NUEVO (11/09/2026) — corrige un vacío real: las estudiantes de un
// Grupo tipo "colegio" (programa Escolar) completan CuestionarioEscolar
// en vez de TestPsicologico (ver sesionController.js), pero nunca
// existió una pantalla para que coordinadora/admin viera esas
// respuestas — el backend (GET /api/cuestionario-escolar) siempre
// funcionó bien, solo faltaba esta UI. Mismo patrón que
// panel/test-psicologico/page.tsx, adaptado a 12 preguntas de escala +
// 2 de reflexión en vez de 54+5.
...
import {
  ...
  SECCIONES,
  PREGUNTAS_REFLEXION,
} from "@/lib/bancoPreguntasEscolar";
...
type Detalle = {
  informacion: { respuestas: number[]; reflexiones: string[]; createdAt: string };
...
export default function PanelCuestionarioEscolarPage() {
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/cuestionario-escolar`, {
  ...
  async function alternarDetalle(userId: string) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/cuestionario-escolar/${userId}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      const json = await res.json();
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-sm text-neutral-text mb-6">
        Respuestas de estudiantes del programa Escolar (colegios) — cuestionario
        informativo, distinto del Perfil conductual que usan el resto de los
        programas. Sin ningún puntaje calculado por el sistema.
      </p>
      ...
      {!cargando && lista.length === 0 && (
        <p className="text-sm text-neutral-text">
          Todavía ninguna estudiante de un grupo Escolar ha completado el
          cuestionario.
        </p>
      )}
            ...
            >
              <div>
                ...
                <p className="text-xs text-neutral-text">
                  {item.userId.cedula || "Sin cédula"} — {item.userId.email}
                </p>
              </div>
                            ...
                            {seccion.preguntas.map((texto, i) => (
                              ...
                              >
                                ...
                                <span className="font-medium text-brand-blue whitespace-nowrap">
                                  {etiquetaDeValor(detalle.informacion.respuestas[offset + i])}
                                </span>
                              </div>
                            ))}
                          </div>
                        ...
                        {PREGUNTAS_REFLEXION.map((pregunta, i) => (
                          <div key={i}>
                            ...
                            <p className="text-sm text-brand-blue mt-1">
                              {detalle.informacion.reflexiones[i] || (
