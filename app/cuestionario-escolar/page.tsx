...
function CuestionarioEscolarContenido() {
  const { token } = useAuth();
    ...
    (async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/cuestionario-escolar/mi-respuesta`,
...
export default function CuestionarioEscolarPage() {
  return (
    <RutaProtegida rolesPermitidos={["estudiante"]}>
      <CuestionarioEscolarContenido />
    </RutaProtegida>
  );
}