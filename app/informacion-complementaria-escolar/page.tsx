...
function InformacionComplementariaEscolarContenido() {
  const { token } = useAuth();
  ...
  const [reflexiones, setReflexiones] = useState<string[]>(Array(2).fill(""));
  const [enviando, setEnviando] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Al entrar, confirmamos si ya lo completó antes — no se puede volver
  // a llenar (una sola vez, igual que TestPsicologico).
  useEffect(() => {
    let cancelado = false;
    (async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/informacion-complementaria-escolar/mi-respuesta`,
    ...
    <main className="bg-neutral-bg min-h-screen px-6 py-12">
      <div className="max-w-2xl mx-auto">
        ...
        {SECCIONES.map((seccion, indiceSeccion) => {
          ...
          // saber la posición real en el array plano de 12.
...
export default function InformacionComplementariaEscolarPage() {
  return (
    <RutaProtegida rolesPermitidos={["estudiante"]}>
      <InformacionComplementariaEscolarContenido />
    </RutaProtegida>
  );
}