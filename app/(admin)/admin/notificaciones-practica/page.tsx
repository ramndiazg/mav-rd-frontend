...
export default function NotificacionesPracticaPage() {
  const { token } = useAuth();
  ...
  async function cargar() {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/destinatarios-practica`,
  ...
  async function guardar(e: React.FormEvent) {
    ...
    try {
      ...
      const url = esNuevo
        ? `${process.env.NEXT_PUBLIC_API_URL}/destinatarios-practica`
        : `${process.env.NEXT_PUBLIC_API_URL}/destinatarios-practica/${editandoId}`;
  ...
  async function toggleActivo(d: Destinatario) {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/destinatarios-practica/${d._id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async function eliminar(d: Destinatario) {
    if (!confirm(`¿Eliminar a "${d.etiqueta}" de la lista de notificaciones de práctica?`)) return;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <p className="text-sm text-neutral-text mb-6">
        ...
        termina toda la teoría y queda lista para la práctica. Lista aparte
        de las notificaciones administrativas (vouchers, balance, empresas).
      </p>
        ...
        <>
          ...
          {!cargando && destinatarios.length === 0 && (
            <p className="text-sm text-neutral-text">
              Todavía no hay nadie configurado para recibir estos avisos.
            </p>
          )}
